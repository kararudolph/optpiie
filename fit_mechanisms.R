...
fit_treat_mech <- function(train_data,
  ...
  if (is.null(valid_data)) {
    ...
    treat_pred_A_star <- train_data$C2 * treat_pred +
      (1 - train_data$C2) * (1 - treat_pred)
    ...
    out_treat_est <- lapply(
      ...
      function(data) {
        ...
        treat_pred_A_star <- data[,"C2"] * treat_pred +
          (1 - data[,"C2"]) * (1 - treat_pred)
...
fit_out_mech <- function(train_data,
  ...
  if (is.null(valid_data)) {
    ...
    C2<-train_data_intervene$C2
    train_data_intervene[, A := C2]
